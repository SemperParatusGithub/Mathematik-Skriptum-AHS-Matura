...
\end{aufgabe}
\begin{loesung} \\
    1) Die Ereignisse, dass eine \emph{rote} oder \emph{blaue Kugel} gezogen wird, schließen einander aus:
    \[
...
\begin{align}
    &P(A\;\mathrm{und}\;B)=P(A\land B) = P(A) \cdot P(B) \qquad \text{(A, B unabhängig)} \\
    &P(A\;\mathrm{und}\;B)=P(A\land B) = P(A) \cdot P(B|A) = P(B) \cdot P(A|B) \qquad \text{(A, B beliebig)}
\end{align}
Hierbei ist $P(B|A)$ die bedingte Wahrscheinlichkeit, also die Wahrscheinlichkeit, dass $B$ eintritt unter der Voraussetzung, dass $A$ schon eingetreten ist. Sie wird durch den Satz von Bayes formuliert. Für einander \emph{ausschließende} Ereignisse gilt dagegen $P(A \land B) = 0$.
...
Die \textbf{Zufallsvariable $X$} ordnet jedem möglichen Ergebnis eines Zufallsexperiments eine reelle Zahl zu. Eine \emph{diskrete Zufallsvariable} nimmt abzählbar viele Werte an (z.B. Binomialverteilung). Eine \emph{stetige Zufallsvariable} kann unendlich viele Werte annehmen (z.B. Normalverteilung).
...
\begin{bsp}
    ...
    \begin{align*}
        P(X<4) &= P(X=1)+P(X=2)+P(X=3) = f(1)+f(2)+f(3) = \\
    ...
    \begin{align*}
        P(3 \leq X \leq 5) &= F(5) - F(3) = \\
        &= \sum_{k=0}^{5} \binom{n}{k} 0.5^k (1-0.5)^{10-k} - \sum_{k=0}^{3} \binom{n}{k} 0.5^k (1-0.5)^{10-k} =\\
        &= \sum_{k=3}^{5} \binom{n}{k} 0.5^k (1-0.5)^{10-k} = 0.56
    \end{align*}
...
\begin{tikzpicture}
  ...
  \begin{axis}[
    ...
    \addplot[red, very thick, smooth] {0.5*(1+erf(x/sqrt(2)))};
...
\begin{equation}
    F(x_) = \int_{-\infty}^{x_0} f(x) dx = \frac{1}{\sqrt{\sigma 2\pi}}
        \int_{-\infty}^{x_0} e^{-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}
\end{equation}
...
\addplot[blue, very thick, smooth] {1/(1.5*sqrt(2*pi))*exp(-x^2/(2*1.5^2))};
\node[below, font=\small] at (axis cs:0,0.0) {$\mu-\sigma$};
\end{axis}
...
\addplot[blue, very thick, smooth] {1/(0.8*sqrt(2*pi))*exp(-x^2/(2*0.8^2))};
\end{axis}
